...
class Social_Links {
	...
	private function get_data() {
		...
		if ( is_singular() || $wp_query->in_the_loop || is_home() || is_category() || is_tag() || is_author() ) {
	...
	private function build_link( $network, $data ) {
		...
		switch ( $network ) {
			...
			case "email":
				return sprintf(
					'<a class="social-share-networks__anchor icon icon-envelope" href="mailto:?subject=%1$s&body=%2$s" title="%3$s"><span%4$s>%3$s</span></a>',
