...
class Nav_Attribute_Filters {
	...
	public function customize_nav_item_start_el( $item_output, $item, $depth, $args ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes ) ) {
			...
			$item_output = sprintf(
				'<%1$s%2$s id="menu-item-%3$s" class="%4$s" data-js="trigger-child-menu" title="%6$s">%5$s %7$s</%1$s>',
				0 === $depth ? 'button' : 'a',
				...
				0 === $depth ? sprintf( '<i class="%s__icon-child-nav icon icon-angle-down" aria-hidden="true"></i>', $args[ 'theme_location' ] ) : ''
			);
		}
