...
const removeErrantPTags = (embed) => {
	...
	if (pStray.length) {
		pStray.forEach((node) => {
			if (!node.classList.contains('wp-embed__p-keep')) {
...
const resetEmbed = () => {
	const embed = document.querySelector('.wp-embed-lazy--is-playing');
...
const playEmbed = (e) => {
	...
	if (document.querySelector('.wp-embed-lazy--is-playing')) {
		resetEmbed();
	}
...
/**
 * @function handleSocialPanel
 * @description Resets playing embed if navigation in social panel.
 */
...
const bindEvents = () => {
	...
	on(document, 'modern_tribe/carousel_panel_slide_changed', handleSocialPanel);
};
